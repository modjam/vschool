...
queryCtrl.controller('queryCtrl', function($scope, $log, $http, $rootScope, geolocation, gservice){
    ...
    geolocation.getLocation().then(function(data){
        ...
        $scope.formData.longitude = parseFloat(coords.long).toFixed(4);
        $scope.formData.latitude = parseFloat(coords.lat).toFixed(4);
    });
...
console.log("hi")
        $scope.$apply(function(){
            $scope.formData.latitude = parseFloat(gservice.clickLat).toFixed(4);
            $scope.formData.longitude = parseFloat(gservice.clickLong).toFixed(4);
    ...
    $scope.queryUsers = function(){
        ...
        $http.post('/routes/query', queryBody)
